            ...
            else
            {
                ...
                do
                {
                    ...
                    try
                    {
                        ...
                        {
                            throw new ArgumentException("Pogresan format! Niste uneli neki parametar ili ste ih uneli previse.");
                        }
                        else if(Int32.TryParse(iscorrectformat[0], out dan) == false || Int32.TryParse(iscorrectformat[1], out mesec) == false || Int32.TryParse(iscorrectformat[2], out godina) == false)
                        {
                            throw new ArgumentException("Pogresan format! Nije moguce pretvoriti neki od parametara u broj. ");
                        }
                        else if (dan > 31 || dan < 0 || mesec > 12 || mesec < 0 || godina > 2021 || godina < 2000)
                        {
                            throw new ArgumentOutOfRangeException("Pogresan format! Neki od podataka ima vrednost vecu/manju nego sto bi smeo da ima.");
                        }
                        else { iscorrect = true; }
                    }
